...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--db-path", default="", help="DB path (default: auto detect db/*7.db)")
...
def main() -> int:
    ...
    for target in targets:
        ...
        for i, recipe in enumerate(recipes, start=1):
            print(
                f"[{target}] {i}/{len(recipes)} window={recipe.window_name} "
                f"lambda={recipe.decay_lambda} feat={recipe.feature_mode} model={recipe.model_name}"
            )
            result = run_recipe(
                dataset=dataset,
