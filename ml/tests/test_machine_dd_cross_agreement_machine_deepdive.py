...
def test_build_full_machine_ranking_keeps_all_machines_and_orders_ranks() -> None:
    from eda import machine_dd_cross_agreement_machine_deepdive as analysis

    residuals = pd.DataFrame(
        [
            _make_residual_row("蒲田7", 7, "m1", 9.0, category="jug"),
            _make_residual_row("蒲田7", 7, "m2", 7.0, category="other"),
            _make_residual_row("蒲田7", 7, "m3", 5.0, category="other"),
            _make_residual_row("蒲田7", 7, "m4", 3.0, category="other"),
            _make_residual_row("蒲田7", 7, "m5", 1.0, category="other"),
            _make_residual_row("蒲田7", 7, "m6", -1.0, category="other"),
        ]
    )
    summary = pd.DataFrame([{"hall": "蒲田7", "dd": 7, "is_novel": True}])
...
def test_compute_machine_dd_self_significance_finds_dd_specific_shift_and_skips_small_cells() -> None:
    from eda import machine_dd_cross_agreement_machine_deepdive as analysis

    rows: list[dict[str, object]] = []
    for i in range(8):
        rows.append({"hall": "蒲田7", "dd": 7, "machine_name": "strong", "hit104": 1})
    for i in range(12):
        rows.append({"hall": "蒲田7", "dd": 1, "machine_name": "strong", "hit104": 0 if i < 10 else 1})
    for i in range(20):
        rows.append({"hall": "蒲田7", "dd": 7, "machine_name": "skip_me", "hit104": 1 if i % 2 == 0 else 0})
    for i in range(20):
        rows.append({"hall": "蒲田7", "dd": 1, "machine_name": "skip_me", "hit104": 0 if i % 2 == 0 else 1})

    frame = pd.DataFrame(rows)

    result = analysis.compute_machine_dd_self_significance(
        "蒲田7",
        frame,
        {7},
    ...
    assert strong["n_at_dd"] == 8
    assert strong["p_value"] < 0.01

    skip_frame = pd.DataFrame(rows[:4])
    skipped = analysis.compute_machine_dd_self_significance(
        "蒲田7",
        skip_frame,
        {7},
        min_machine_days_total=10,
...
def test_apply_fdr_per_hall_dd_is_independent_by_hall_and_dd() -> None:
    from eda import machine_dd_cross_agreement_machine_deepdive as analysis
    ...
    result = analysis.apply_fdr_per_hall_dd(self_sig, fdr_alpha=0.05)
...
def test_find_repeat_contributors_tracks_multiple_dds() -> None:
    from eda import machine_dd_cross_agreement_machine_deepdive as analysis

    rows: list[dict[str, object]] = []
    for dd, residual in [(7, 9.0), (8, 8.0), (9, 7.0)]:
        rows.append(
            {
                "hall": "蒲田7",
                "dd": dd,
    ...
    assert row["dds_appeared"] == "7;8;9"
    assert row["n_machine_significant_appearances"] == 3


def test_find_cross_hall_consistency_records_exact_and_mismatch_dds() -> None:
    from eda import machine_dd_cross_agreement_machine_deepdive as analysis
...
def test_main_writes_requested_csvs(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    fake_outputs = {
        "蒲田7": {
            "residuals": pd.DataFrame(
                [
                    _make_residual_row("蒲田7", 7, "shared", 9.0, category="jug"),
                    _make_residual_row("蒲田7", 7, "other1", -1.0, category="other"),
                    _make_residual_row("蒲田7", 8, "shared", 8.0, category="jug"),
                    _make_residual_row("蒲田7", 8, "other2", -2.0, category="other"),
                ]
            ),
            "summary": pd.DataFrame(
                [{"hall": "蒲田7", "dd": 7, "is_novel": True}, {"hall": "蒲田7", "dd": 8, "is_novel": False}]
            ),
        }
    }
    ...
    monkeypatch.setattr(analysis, "build_hall_outputs", fake_build_hall_outputs)
