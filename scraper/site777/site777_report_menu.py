# -*- coding: utf-8 -*-
"""報告メニュー1〜7を固定順で出す（スキル site777-highsetting-scan の手順5）。
...
2026-09-24 追記: メニュー1〜5はAT機の差枚指標に偏っており、ノーマル/BT/A+AT機の
RB確率分析(判別可能機種)がメニュー番号を持たないままブリーフ後方に埋もれ、
報告のたびに後回しにされる事故が繰り返された(memory: feedback-normal-machines-never-secondary)。
6・7として番号を振り、site777_live_brief.py 側で produced に追加してもらう
ことで、欠けたら他のメニューと同じく終了コード2にする。

出せない項目は空欄にせず「未出力（理由）」と書く。8（予告突合）は site777_live_brief.py 側。
"""
...
REQUIRED = (
    ...
    "4. ニブイチ判定",
    "5. 全台系判定（全台合算）",
    "6. 判別可能機種（ノーマル/BT/A+AT）RB確率ランキング",
    "7. AT機G比×平均差枚",
)
