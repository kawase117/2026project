                ...
                for date_info in result['detailed_log']['dates']:
                    if date_info['status'] == 'failed':
                        ...
                        failed_records.append(
                            {'hall_name': hall_name, 'date': formatted_date, 'reason': date_info.get('reason', '不明')}
                        )
            ...
            try:
                ...
                if not click_success:
                    ...
                    detailed_log['dates'].append(
                        {'date': date_str, 'status': 'failed', 'reason': click_reason or 'リンククリック失敗'}
                    )
                ...
                if success:
                    ...
                    if os.path.exists(json_filepath):
                        ...
                        if db_success:
                            ...
                            detailed_log['dates'].append({'date': date_str, 'status': 'success', 'reason': None})
                            consecutive_failures = 0
                        else:
                            print(f"   [WARN] {date_str}: 取得成功、DB保存失敗")
                            success_dates.append(date_str)
                            detailed_log['dates'].append(
                                {'date': date_str, 'status': 'success', 'reason': 'DB保存失敗（JSONは取得済み）'}
                            )
                            consecutive_failures = 0
                    else:
                        ...
                        failed_dates.append(date_str)
                        detailed_log['dates'].append(
                            {'date': date_str, 'status': 'failed', 'reason': 'JSONファイル未検出'}
                        )
                        consecutive_failures += 1
                else:
                    print(f"   [ERROR] {date_str}: データ抽出失敗")
                    failed_dates.append(date_str)
                    detailed_log['dates'].append({'date': date_str, 'status': 'failed', 'reason': 'データ抽出失敗'})
                    consecutive_failures += 1
                ...
                failed_dates.append(date_str)
                detailed_log['dates'].append({'date': date_str, 'status': 'failed', 'reason': f'例外エラー: {str(e)}'})
                consecutive_failures += 1
    ...
    try:
        ...
        for i, hall in enumerate(halls, 1):
            ...
            try:
                ...
                error_log = {'hall_name': hall_name, 'status': 'error', 'error_message': str(e), 'dates': []}
