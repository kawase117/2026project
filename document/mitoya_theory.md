...
> **最終更新**: 2026-09-10（EVENT_DDS減衰・強ゾロ目追試・指標選択を追記）／骨格は2026-07-02（再構築版)。旧版は`eda/mitoya_phase5b_theory.py`が自動生成するPhase3〜8の生データ羅列と、Phase10以降に手作業で追加された高品質な分析（否定仮説カタログ・アンチパターン7類型・蒲田7比較・実運用フロー）が同居していた。本版はPhase3〜8の生データ羅列を評価bundleへ切り離し、Phase10〜12の内容を骨格として、Instinct全件・kamata7_theory.md相当の構成に再編した。
...
| **X_DDS（イベント日）** | DD in {4, 7, 14, 17, 24, 27}（実運用フローでは1,30も含めた拡張版を使用） | `is_x_day`相当 | 一般的に想定される{1,11,22,25,30,31}という定義は**誤り**。実データから逆引きして確定。⚠️`eda/core.py`の`HALL_EVENT_DIGITS["みとや"]`はベースの6個のみで1,30が欠落したまま（2026-08-01棚卸しで確認、`eda/briefing_common.py`経由の分析はこの欠落版を使う） |
...
**【2026-08-01 実測】カテゴリ別DD残差とOKIの月末・強ゾロ目効果**

機種内残差pp（基準は機種の全期間平均、楽園theory §3.1aと同一手法）でJUG/AT一般を再確認:

| 区分 | 上位DD | 下位DD |
|---|---|---|
| JUG（n中央値1369） | 14(+0.74), 4(+0.73★), 5(+0.61), 27(+0.59★) | 22(-0.69), 23(-0.73), 29(-0.83) |
| AT一般（n中央値1558） | 4(+3.02★), 24(+2.87★), 14(+2.33), 17(+2.23★) | 13(-2.37), 26(-2.76), 25(-3.52) |

AT一般ではX_DDS内のDD(4,24,17)が上位を占め既存定義とよく整合する。JUGは効果量が全体的に小さく(最大+0.74)、既存の「DD軸>>曜日軸」ほど明確な軸ではない可能性がある。

**OKIカテゴリで月末+6.04pp・強ゾロ目+5.57ppという、みとや全カテゴリ中最大の効果**を検出（n要確認、他ホールのOKIは軒並み1pp未満）。OKI固有の投入パターンの可能性があり、今回初めて見つかった軸のため未検証。追試が必要。

⚠️【2026-08-01 同日中に追記した方法依存の注意】上表は**機種内残差法**（各機種の全期間平均を基準）による**全期間**集計。みとやでは未検証だが、以下2つの弱点がある。

1. **手法依存**: カテゴリ平均残差法で計算し直すと上位DDが再現しないことが蒲田7で判明（→`kamata7_theory.md` 2.8b節の訂正）。単一手法の結果を確定として扱わない。
2. **全期間平均が直近のレジーム変化を消す**: 蒲田1のDD1で実証済み（→`kamata1_theory.md`）。X_DDSは実データから逆引きして確定した経緯があるが、それも当時の期間での話であり、定期的な四半期別の再確認が要る。
...
## ⚠️ EVENT_DDS も 2026-04-27 で減衰した（2026-09-10 追記）

**角番シグナルと同じ破断日で、DD の効きも半減している。** 上の角番の節は
「店長交代に伴う設定投入方針の変化」と結論しているが、影響は角番だけではなかった。

台単位の機械割（**非加重**）のホール平均からの差 pp、括弧内は勝率差:

| DD | 2025年 | 2026-01〜04/26 | **2026-04/27以降** | 判定 |
|---|---|---|---|---|
| 14 | +2.19 (+6.1) | +3.59 (+8.3) | **+3.13 (+5.2)** | 維持・最強 |
| 27 | +1.16 (+3.7) | +0.48 (+0.7) | **+1.60 (+2.9)** | 維持 |
| 17 | +1.30 (+3.4) | +0.53 (+2.5) | **+1.11 (+1.1)** | 維持 |
| 7 | +2.13 (+4.2) | +1.61 (+4.3) | **+1.08 (+4.0)** | 減衰 |
| 4 | +3.33 (+7.7) | +2.31 (+6.3) | **+0.88 (+2.9)** | 大幅減衰 |
| 24 | +2.74 (+6.2) | +2.11 (+2.8) | **+0.33 (-0.0)** | ほぼ消滅 |
| 30 | +1.52 (+1.8) | +3.38 (+4.1) | **+0.00 (-0.5)** | 消滅 |
| **1** | -1.09 (-4.1) | +0.19 (+1.2) | **-2.24 (-4.5)** | **反転** |

EVENT_DDS 全体の優位: **+2.28pp → +2.42pp → +1.18pp**（対 非EVENT）。約半減。

### 運用上の扱い

1. **DD1 を EVENT_DDS として扱わない。** 2025年から既に負（-1.09）で、直近は -2.24。
   `mitoya-event-definition-confirmed`（DD{1,4,7,14,17,24,27,30}で104%超え31.5%）は
   全期間集計であり、DD1 の寄与は当時から負だった可能性が高い
2. **DD24・DD30 は現時点で根拠を失っている。** 優先度を落とす
3. **生き残っているのは DD14・DD27・DD17・DD7。** 特に DD14 は交代後も +3.13pp で最強
4. 定義の改訂は 2026-04-27 以降のデータのみで再測定してから行う（角番の節と同じ方針）

出典: `eda/hall_deployment_tendency.py`（2026-09-10）

---

## 強ゾロ目（MM=DD）の追試 — OKI は再現するが n 不足、AT等のほうが堅い（2026-09-10 追記）

上の【2026-08-01 実測】が「OKIカテゴリで強ゾロ目+5.57pp、**n要確認・未検証・追試が必要**」
としていた件の追試。稼働を統制（強ゾロ目と同じ G/台 帯の非ゾロ目日を対照）し、
台単位の機械割を**非加重**で平均した。

| 区分 | 全期間（ゾロ目21日 / 対照258日） | 2026-04以降（6日 / 15日） |
|---|---|---|
| **OKI** | **+3.76pp**（n=138 vs 1,774） | **+6.45pp**（n=20 vs 22） |
| AT等 | **+2.07pp**（n=3,176 vs 38,734） | **+3.75pp**（n=951 vs 2,345） |
| HANA | +3.29pp（n=21） | +3.77pp（n=15） |
| JUG | +0.40pp | +0.56pp |
| BT | -1.09pp | +1.15pp |
| 全体 | +1.35pp | +2.61pp |

- **OKI は向きも大きさも再現する。** ただし原記述の「n要確認」はそのまま残る
  （全期間で138台日、直近は20台日）
- **効果は OKI 固有ではない。** AT等が n=3,176 で +2.07pp、直近 n=951 で +3.75pp。
  「OKI固有の投入パターン」という解釈は狭い。JUG だけがほぼ動かない（+0.4〜0.6pp）
- 稼働統制前のホール全体は +2.72pp、統制後 +1.35pp。**上昇の約半分は稼働由来**

⚠️ 他ホールでも同じ軸を測ると 9/10ホールで正だが、稼働を統制すると
雑色 +2.59pp・みとや +1.90pp 以外はほぼ消える。**ホール横断でプールしないこと。**

---

## 指標の選び方 — 単一指標で曜日を語らない（2026-09-10 追記）

**出率（総獲得/総投入）は回転数の加重平均**なので、低設定台が多く回された日は下がる。
土日は平日勤めの層が増えて回し方が変わるため、高設定が増えても出率には出にくい。
...
同じ日が、指標によって「平常」にも「明確に強い」にも見える。**乖離そのものが
『入っているが回され方で薄まった』の指標**である。みとやの土曜も同型
（出率+0.03 / 機械割+0.62 / 勝率+1.6 / 中央差枚 -47）。

⚠️ **104%率を回転数帯で揃えるのはやりすぎると選択を持ち込む。** 回転数は設定への
反応でもあるので、帯で切ると「よく回された高設定台」が高稼働日から落ちる。
参考値に留め、主判定に使わない。既存の「hit104率を主指標にしてはならない」
（`Var(機械割) ∝ 1/games`）は**低回転側の偽陽性**についての警告で、これとは別問題。

出力は `eda/hall_deployment_tendency.py` が6指標を同時に出す。
