...
from eda.mitoya_umj_mister_common import DD19_29_MODELS, RESULT_DIR, analyze_dd_candidate, load_frame
...
def _candidate_row(model: str, candidate: dict[str, object]) -> dict[str, object]:
    row: dict[str, object] = {
        "model": model,
        "dd": int(candidate["dd"]),
    ...
    for label, jp in [
        ...
        row[f"weekday_{label}_rate"] = weekday_rates.get(jp, pd.NA)

    weekend_rates = _rate_from_lookup(candidate["weekday_weekend_df"], "bucket", "pooled_rb_rate")
    row["weekday_weekday_rate"] = weekend_rates.get("weekday", pd.NA)
    row["weekday_weekend_rate"] = weekend_rates.get("weekend", pd.NA)
...
def main() -> int:
    ...
    for model in DD19_29_MODELS:
        ...
        for dd in TARGET_DDS:
            candidate = analyze_dd_candidate(frame, dd)
            candidate_rows.append(_candidate_row(model, candidate))
            print(
                f"  DD{dd}: rate={candidate['target_pooled_rb_rate']:.6f}, rest={candidate['rest_pooled_rb_rate']:.6f}, "
                f"p={candidate['dd_vs_rest_p']:.4g}, verdict={candidate['overall_verdict']}"
            )

    candidate_df = pd.DataFrame(candidate_rows)
    _write_csv(candidate_df, result_dir / "task_d_dd19_dd29_summary.csv")

    print(f"written: {result_dir / 'task_d_dd19_dd29_summary.csv'}")
    return 0
