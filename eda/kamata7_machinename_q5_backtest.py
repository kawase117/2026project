...
OUTPUT_DIR = Path(__file__).resolve().parents[1] / "tmp" / "kamata7_machinename_q5_backtest"
MIN_GAMES = 1000
XDAY_DAYS = {7, 17, 27}
    ...
    *,
    ...
    for period_train, period_test in zip(periods[:-1], periods[1:], strict=True):
        ...
        if floor is not None:
            test = test[test["floor"] == floor].copy()
        if xday_only:
            test = test[test["is_xday7"]].copy()
        ...
        if xday_only:
            row["n_xday_dates"] = float(test["date"].nunique())
        row.update(
            summarize_pair(
                test,
                q5_names,
    ...
    if xday_only:
        columns = ["period_train", "period_test", "n_xday_dates"] + columns[2:]
...
def build_backtest_outputs(df: pd.DataFrame) -> dict[str, pd.DataFrame]:
    ...
    if "floor" not in prepared.columns:
        prepared["floor"] = prepared["machine_number"].apply(lambda x: "2F" if x < 3000 else "3F")
    if "is_xday7" not in prepared.columns:
        if not pd.api.types.is_datetime64_any_dtype(prepared["date"]):
            prepared["date"] = pd.to_datetime(prepared["date"], format="%Y%m%d")
        prepared["is_xday7"] = prepared["date"].dt.day.isin(XDAY_DAYS)

    training_table = build_training_table(prepared)
    return {
        "period_summary": build_period_summary(prepared, training_table),
        "period_summary_2F": build_period_summary(prepared, training_table, floor="2F"),
        "period_summary_3F": build_period_summary(prepared, training_table, floor="3F"),
        "xday7_overlay": build_period_summary(prepared, training_table, xday_only=True),
...
def build_report(outputs: dict[str, pd.DataFrame]) -> str:
    ...
    report_lines = [
        "# Kamata7 machine_name Q5継続バックテスト",
        "",
        "## 全ペア平均比較",
        *header,
        *rows,
    ...
    if "n_xday_dates" in outputs["xday7_overlay"].columns:
        report_lines.append(f"- 7系日の平均対象日数/ペア: {_fmt(_mean_metrics(outputs['xday7_overlay']).get('n_xday_dates'))}")
    return "\n".join(report_lines) + "\n"
...
def save_outputs(outputs: dict[str, pd.DataFrame], output_dir: Path) -> None:
    ...
    outputs["period_summary_2F"].to_csv(output_dir / "period_summary_2F.csv", index=False, encoding="utf-8-sig")
    outputs["period_summary_3F"].to_csv(output_dir / "period_summary_3F.csv", index=False, encoding="utf-8-sig")
    outputs["xday7_overlay"].to_csv(output_dir / "xday7_overlay.csv", index=False, encoding="utf-8-sig")
