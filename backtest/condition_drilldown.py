...
- **判定は絶対差枚で行う。相対差枚（ホール平均との差）を使わない。** 条件日は
  ホール全体も上がるので相対では相殺され、効果を過小評価する。蒲田1のBTを相対で見て
  「ボーナスは増えているのに勝てない」と誤った結論を出しかけた。絶対で見れば
  ボーナスは正しく出玉に変換されている（ボーナス1%あたり 48〜69枚、3カテゴリで同等）
- **平均ではなく分布で見る。** 店は全台を少し上げるのではなく一部を高設定にする。
  蒲田7の強ゾロ目は平均設定 +0.24 に対し高設定率 17.9%→26.9%（1.50倍）、
  差枚も上位10%が +321枚 動くのに平均は +56枚しか動かない
