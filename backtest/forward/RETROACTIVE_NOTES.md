...
## 2026-09-13 凍結タイミングの変更により 20260914 分を破棄（21件）

`plan-all` は対象日 0 時を締め切りにしていたため、朝の定例は翌日分しか凍結できず、
選択は常に対象日の2日前のデータで作られていた（例: 20260913 分は data_asof=20260911）。
バックテストは前日までのデータを前提にしているので、フォワードだけが劣化版を検証していた。

締め切りを対象日 16:00 JST（`forward.FREEZE_DEADLINE_HOUR`）に移し、朝の定例は
**当日分** を前日までのデータで凍結する方式に変えた。前日分が未取込でも締め切り前なら
凍結し、遅れは `data_lag_days`（plan 本体と RUNS の各 disposition）と
`max_data_lag_days`（RUNS の実行記録）に残す。

2026-09-13 07:34 に data_asof=20260912 で凍結済みだった 20260914 分の21件は
`invalidated/` へ移し、理由を `invalidated/NOTES.jsonl` に記録した。対象日未到来で
結果は一切見ていない。2026-09-14 朝の定例で data_asof=20260913 により再凍結する。
LEDGER には破棄前の digest が残るが、同じキーの後の行が採用される（`_ledger_entries`）。

集計上の注意: 2026-09-13 以前の plan はすべて「2日遅れ」、以降は原則「1日遅れ」。
`data_lag_days` が無い旧 plan は `target_date - data_asof` で復元できる。両者を合算しない。
