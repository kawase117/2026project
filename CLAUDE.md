...
## 実行環境の自動判定（2026-08-26 追加）

指示がなくても、セッション開始時に作業環境を自動判定し、振る舞いを切り替えること。

- **判定基準**：作業ディレクトリがWindowsパス（`C:\Users\...`）でない、または
  システムプロンプトに「managed remote execution environment」等コンテナ/使い捨て環境である旨の記載がある場合 → **リモート実行環境**と判定する。
  それ以外（ローカルWindows環境）は従来通り。

- **リモート実行環境と判定した場合のデフォルト動作**：
  - 「起動方法」「Python実行環境の固定」節のWindows/venv向け手順（`venv\Scripts\python.exe` 等）は適用しない。環境に存在するpython/streamlitをそのまま使う
  - 修正が完了したら、GitHub節の「プッシュしますか？」確認は**省略し**、指定ブランチへ自動でpush・PR（draft）作成まで行う
    - 理由：リモートコンテナは非活動時に破棄されるため、確認待ちのまま放置すると作業が失われる
  - PR作成後は `subscribe_pr_activity` で監視を自動開始する

- **ローカルWindows環境と判定した場合**：従来通り「プッシュしますか？」の確認を必須とする（GitHub節参照）
...
ブランチ: main
修正完了後、ユーザーに「プッシュしますか？」と確認してからプッシュする（ローカルWindows環境でのデフォルト。リモート実行環境の場合は上記「実行環境の自動判定」節を参照）。
