...
✅ **`rank_from_min`/`rank_from_max` の `rank == 1` は物理的な島の端で正しい**（楽園・みとや・蒲田7）。

かつて「2列島では `rank==1` が片端の2台しか拾わない」という記述をここに置いていたが、
それは `backtest/results/regime/FINDINGS.md` 追試11 の**初版の診断**であり、
**同じ追試の補遺（2026-07-24）が自ら否定して修正済み**である。真の原因は順位の
算出方法ではなく、**`section` 定義が背中合わせ2列の島を1つにまとめていたこと**だった。
`database/migrate_split_rakuen_sections.py` で実配置どおりに分割済み。

| エポック | section数 | 2列島 | rank端 | 物理端 | 取りこぼし |
|---|---|---|---|---|---|
| 楽園 工事前 修正前 | 43 | 17 | 86 | 130 | 45 |
| 楽園 工事前 修正後 | **63** | 1※ | 126 | 127 | **3** |
| 楽園 工事後 修正前 | 43 | 18 | 86 | 126 | 40 |
| 楽園 工事後 修正後 | **64** | 1※ | 128 | 129 | **1** |

※ 残る1件は三角形配置の `2126-2128`（仕様通り分割しない）

2026-09-10 に現行DBで再確認: 楽園の現行エポックは70セクション中69が1列、
座標由来の物理端141台に対し `rank==1` が140台で **99%一致**。
**素朴に `min(rank_from_min, rank_from_max) == 1` を端として使ってよい。**
...
ホール別の状況: 楽園・みとや・蒲田7 は `rank==1` で端が取れる
（蒲田7 の主軸は `rank_from_aisle`）/
**蒲田1=x,y が台番号に沿った対角線状（座標重複0・30セクション中14が完全対角）で
合成座標の疑いが濃厚。座標ベースの位置検証は不可**（`rank` 側を使うこと）。
