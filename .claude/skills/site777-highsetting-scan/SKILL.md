...
### 2.5 差枚・カウントの品質フラグを読む（2026-09-14 追加）

`site777_analysis*.json` の各台と `diff_quality` に次のフラグが付く。報告前に必ず見る。

- **`diff_censored_high` / `diff_censored_low`** — グラフ縦軸は **±5,000枚固定**。終点が上端/下端に
  張り付いた台は真の差枚が軸の外にある（9/13 19:25 は267台中12台、2119は最高出玉11,415枚）。
  `latest_diff` には±5,000が入り、レポートは「≥+5,000」「≤-5,000」と出す。
  これを含む合計・平均差枚（`diff_bound`）は **lower=下限 / upper=上限 / indeterminate=向き不明**。
  「+4,940」を実測値として並びブロック合計や機種平均に使わない。勝敗は符号が確定するので勝率は影響なし
- **`axis_label_corrected`** — 軸ラベルのOCR誤読（2119 が「5,000」を「500」と読み、上端張り付きなのに
  +600 と出た）。ラベル間隔が既知の約318pxと合うときだけ固定軸で補正済み。
  **`diff_unreliable`** は間隔も合わず縮尺を信用できない台で、差枚は参考値。実画像を Read で見る
- **`counts_duplicate_suspect`** — 同一機種内で累計G・BB・RB・ARTが完全一致（1,000G以上）。
  9/13 のマイジャグV 1202/1215 は一覧でも台別グラフページのランプでも 2,306G/BB6/RB5 だったが、
  現在G・最高出玉・推移線は別物で、**サイト側のカウンタ重複**（パーサでは直せない）。
  設定推定の信用度は「要確認(カウント重複)」に落ち、全台系判定表には除外時の判定が併記される。
  どちらの台の値が本物か分からないので、両方とも設定の根拠にしない
- 判定ロジックは `scraper/site777/graph_quality.py`。保存済み metrics からも再計算されるので、
  古い `site777_graph_metrics_filtered.json` を `site777_analyze.py` に通すだけでフラグが付く。
  ana-slo DB の +15,477 クリップ（`backtest/censoring.py`）とは別物
...
2026-09-16 にこれらをユーザーから都度指摘されて出していたため、`site777_live_brief.py` に固定した。
