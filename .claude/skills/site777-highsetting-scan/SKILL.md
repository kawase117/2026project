...
**全台系の判定は機種の全台を合算する**（2026-09-13 追加）: 1台ずつでは設定を絞れない回転数でも、
機種の全台を合算すれば判定できることがある。設定レポート末尾の
「機種別 全台系判定（全台合算）」表（`setting_estimator.judge_model_zentaikei`）を見る。
合算には2,000G未満の台も含む。**合算平均が高いだけで「全」と言わない。** 1〜2台の突出
（機種イチの撒き餌）でも合算平均は上がるため、次の2つを併せて見る:
- 全高vs全低（全台同一設定を仮定した合算尤度の5-6側/1-4側比）が10倍以上
- 高設定台数 k の事後分布で k=全台が最尤かつ P(全台高)≥50%

実例: 9/13 楽園のアイム62台は合算RBが設定1比 p<0.0001 だったが、全台高設定はほぼ否定され
「62台中15台前後の部分投入」だった。昼の段階ではほとんどの機種が「判定保留（情報不足）」になる。
これは正常で、閉店近くの再収集で判定できる機種が増える。
...
5. **設定推定**（手順4）— 高低比降順、幅・信用度・差枚を併記。台別で絞れない機種は
   **機種別 全台系判定（全台合算）** の判定列を読む（「判定保留」は情報不足であって否定ではない）
